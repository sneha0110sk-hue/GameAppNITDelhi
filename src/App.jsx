...
// --- STYLES (Injected directly to avoid file resolution errors) ---
const styles = `
/* --- The Casino Table Texture --- */
.game-table {
...
// --- CONFIGURATION ---
// IMPORTANT: If the screen is blank, ensure your keys are pasted below.
...
try {
  ...
  console.log('Environment config not found.');
}

// IF CONFIG IS MISSING, PASTE YOURS HERE:
if (!firebaseConfig) {
  firebaseConfig = {
...
const getSuitIcon = (suit, size = 16) => {
  ...
  switch (suit) {
    case 'H':
      return <Heart size={size} className={`${className} fill-current`} />;
    case 'D':
      return <Diamond size={size} className={`${className} fill-current`} />;
    case 'S':
      return <Spade size={size} className={`${className} fill-current`} />;
    case 'C':
      return <Club size={size} className={`${className} fill-current`} />;
    default:
      return null;
  }
};
...
const Card = ({
  ...
  if (faceDown) {
    return (
      <div
        ...
        className={`
          card-base card-pattern rounded-md border border-white/50
  ...
  if (!card)
    return (
      <div
        className={`${
          isSmall ? 'w-6 h-9 md:w-8 md:h-12' : 'w-12 h-16 md:w-20 md:h-28'
        } border-2 border-dashed border-white/20 rounded-md`}
      ></div>
    );
  ...
  return (
    <div
      ...
      className={`
        card-base bg-white rounded-md flex flex-col items-center justify-between p-0.5 md:p-1 select-none relative
        ${
          isSmall
            ? 'w-6 h-9 md:w-8 md:h-12 text-[8px] md:text-xs'
            : 'w-12 h-16 md:w-20 md:h-28 text-xs md:text-base'
        } 
        ${
          playable
            ? 'cursor-pointer card-hover ring-2 md:ring-4 ring-yellow-400 z-10'
            : ''
        }
      `}
    >
...
export default function App() {
  ...
  // Auth Init
  useEffect(() => {
    if (!auth) return; // Prevent run if auth not init
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      doc(db, 'artifacts', APP_ID, 'public', 'data', 'games', gameId),
      (snap) => {
  ...
  const createGame = async () => {
    ...
    await setDoc(
      doc(db, 'artifacts', APP_ID, 'public', 'data', 'games', newId),
      {
        id: newId,
        hostId: user.uid,
        status: 'LOBBY',
  ...
  const joinGame = async () => {
    ...
    if (
      ...
      !data.players.find((p) => p.uid === user.uid)
    ) {
  ...
  const startGame = async () => {
    ...
    players.forEach((p, i) => {
      const s = i * 8;
      p.faceDown = deck.slice(s, s + 3);
      p.faceUp = deck.slice(s + 3, s + 6);
      p.hand = deck.slice(s + 6, s + 8);
    });
    await updateDoc(
      doc(db, 'artifacts', APP_ID, 'public', 'data', 'games', gameId),
      {
  ...
  const makeBid = async (amount) => {
    ...
    if (amount === 0) {
      ...
      let updates = {
        'bid.passedPlayers': passed,
        currentTurnIndex: (gameState.currentTurnIndex + 1) % 6,
      };
      ...
      await updateDoc(ref, {
        'bid.currentHighBid': amount,
        'bid.currentHighBidder': myIndex,
        currentTurnIndex: (gameState.currentTurnIndex + 1) % 6,
      });
    }
  };
  ...
  const selectMasterSuit = async (suit) => {
    await updateDoc(
      doc(db, 'artifacts', APP_ID, 'public', 'data', 'games', gameId),
      {
        'bid.suit': suit,
        status: 'PLAYING',
        currentTurnIndex: gameState.bid.winnerIndex,
      }
    );
  };
  ...
  const playCard = async (card, source) => {
    ...
    const leadSuit =
      gameState.trick.length > 0 ? gameState.trick[0].card.suit : null;
    const hasVisibleLead = leadSuit
      ? [...player.hand, ...player.faceUp].some((c) => c.suit === leadSuit)
      : false;

    if (
      source === 'faceDown' &&
      (player.hand.length > 0 || player.faceUp.length > 0) &&
      !(!hasVisibleLead && leadSuit)
    ) {
      alert('Cannot play blind card yet!');
      return;
    }
    if (
      source !== 'faceDown' &&
      ...
      alert(`Must follow suit (${leadSuit})`);
      return;
    }

    const updatedPlayer = { ...player };
    updatedPlayer[source] = updatedPlayer[source].filter(
      (c) => c.id !== card.id
    );
    const newTrick = [...gameState.trick, { card, playerIndex: myIdx }];

    let updates = {
      [`players.${myIdx}`]: updatedPlayer,
      trick: newTrick,
      currentTurnIndex: (gameState.currentTurnIndex + 1) % 6,
    };
    ...
    if (newTrick.length === 6) {
      ...
      for (let i = 1; i < 6; i++) {
        ...
        if (c.suit === trump && winCard.suit !== trump) {
          winCard = c;
          winIdx = i;
        } else if (c.suit === winCard.suit && c.value > winCard.value) {
          winCard = c;
          winIdx = i;
        }
      }
  ...
  const mySeatIndex = useMemo(
    () => gameState?.players.findIndex((p) => p.uid === user?.uid) ?? -1,
    [gameState, user]
  );
  const getPlayer = (offset) =>
    gameState ? gameState.players[(mySeatIndex + offset) % 6] : null;

  // SAFETY CHECK: If keys are missing, show instructions instead of crashing
  if (!app) {
    return (
      <div className="min-h-screen bg-gray-900 text-white flex flex-col items-center justify-center p-8 text-center">
        ...
        <p className="max-w-md mb-6">
          The game cannot start because the Firebase Keys are missing in the
          code.
        </p>
        <div className="bg-gray-800 p-6 rounded-lg text-left w-full max-w-lg overflow-x-auto">
          <p className="text-sm text-gray-400 mb-2">
            1. Open <code>App.jsx</code>
          </p>
          <p className="text-sm text-gray-400 mb-2">
            2. Find the line:{' '}
            <code>// IF CONFIG IS MISSING, PASTE YOURS HERE:</code>
          </p>
          <p className="text-sm text-gray-400 mb-4">
            3. Paste your config inside the object like this:
          </p>
  ...
  if (!user)
    return (
      <>
        <style>{styles}</style>
        <div className="game-table min-h-screen text-white flex items-center justify-center">
          Connecting...
        </div>
      </>
    );
      ...
      <>
        ...
        <div className="game-table min-h-screen text-white flex flex-col items-center justify-center p-4">
          <div className="glass-panel p-6 md:p-8 rounded-2xl max-w-md w-full text-center">
            <h1 className="text-3xl md:text-5xl font-serif text-gold mb-6 md:mb-8 drop-shadow-lg">
              Royal Court
            </h1>
            {!gameState ? (
              <div className="space-y-4">
                <button
                  onClick={createGame}
                  className="w-full bg-gold text-black font-bold py-3 rounded-lg hover:brightness-110 shadow-lg transition"
                >
                  Create Table
                </button>
                <div className="flex gap-2">
                  <input
                    type="text"
                    placeholder="CODE"
                    className="flex-1 bg-black/40 border border-white/20 rounded px-3 text-center uppercase"
                    value={gameId}
                    onChange={(e) => setGameId(e.target.value.toUpperCase())}
                  />
                  <button
                    onClick={joinGame}
                    className="glass-btn px-4 md:px-6 py-2 rounded font-bold"
                  >
                    Join
                  </button>
                </div>
              ...
              <div className="space-y-6">
                <div className="bg-black/40 p-4 rounded-xl border border-white/10">
                  ...
                  <div className="text-2xl md:text-3xl font-mono text-gold tracking-widest select-all">
                    {gameState.id}
                  </div>
                </div>
                <div className="space-y-2 max-h-48 overflow-y-auto">
                  {gameState.players.map((p, i) => (
                    <div
                      key={i}
                      className="flex justify-between items-center bg-white/5 p-2 rounded text-sm md:text-base"
                    >
                      {isEditingName && p.uid === user.uid ? (
                        <div className="flex gap-2 flex-1 items-center">
                          <input
                            ...
                            className="flex-1 bg-black/50 border border-white/30 rounded px-2 py-1 text-white"
                            autoFocus
                          />
                          <button
                      ...
                      <span
                        className={`text-xs px-2 py-0.5 rounded ${
                          p.team === 'A' ? 'bg-red-900/50' : 'bg-blue-900/50'
                        }`}
                      >
                        Team {p.team}
                      </span>
                ...
                {gameState.hostId === user.uid && (
                  <button
                    ...
                    onClick={startGame}
                    className="w-full bg-green-600 disabled:bg-gray-600 py-3 rounded font-bold shadow-lg text-sm md:text-base"
                  >
  ...
  // GAME
  // Responsive Positions using Tailwind MD/LG breakpoints
  // Mobile: Tighter layout. Desktop: Expanded layout.
  const positions = [
    'bottom-4 md:bottom-8 left-1/2 -translate-x-1/2 z-30', // Me (Bottom Center)
    'bottom-28 right-0 md:bottom-32 md:right-8 scale-75 md:scale-100 origin-bottom-right', // Player 1 (Bottom Right)
    'top-28 right-0 md:top-32 md:right-8 scale-75 md:scale-100 origin-top-right', // Player 2 (Top Right)
    'top-14 md:top-8 left-1/2 -translate-x-1/2 origin-top', // Player 3 (Top Center)
    'top-28 left-0 md:top-32 md:left-8 scale-75 md:scale-100 origin-top-left', // Player 4 (Top Left)
    'bottom-28 left-0 md:bottom-32 md:left-8 scale-75 md:scale-100 origin-bottom-left', // Player 5 (Bottom Left)
  ];
      ...
      <div className="game-table text-white font-sans select-none">
        ...
        <div className="absolute top-0 left-0 right-0 p-2 md:p-4 flex justify-between items-start z-50 pointer-events-none">
          <div className="glass-panel p-2 md:p-3 rounded-xl pointer-events-auto">
            <div className="text-[10px] md:text-xs text-gray-400 uppercase tracking-wider mb-1">
              Score
            </div>
            <div className="flex gap-3 md:gap-6 font-serif text-sm md:text-xl">
              <span className="text-red-300">
                A: <b className="text-white">{gameState.scores.A}</b>
              </span>
              <span className="text-blue-300">
                B: <b className="text-white">{gameState.scores.B}</b>
              </span>
            </div>
          </div>
          ...
          {gameState.bid.suit && (
            <div className="glass-panel px-4 py-1 md:px-6 md:py-2 rounded-b-xl md:rounded-b-2xl -mt-2 md:-mt-4 flex flex-col items-center pointer-events-auto border-t-0 shadow-[0_10px_30px_rgba(0,0,0,0.5)]">
              <div className="text-gold text-[10px] md:text-xs uppercase mb-1">
                Master
              </div>
        ...
        <div className="absolute inset-0 flex items-center justify-center perspective-[1000px]">
          ...
          {[0, 1, 2, 3, 4, 5].map((offset) => {
            ...
            return (
              <div
                key={offset}
                className={`absolute ${positions[offset]} flex flex-col items-center transition-all duration-500`}
              >
                {/* Avatar */}
                <div
                  className={`
                  relative mb-2 md:mb-4 transition-all duration-300
                  ${
                    isActive
                      ? 'scale-110 md:scale-125 z-40'
                      : 'scale-90 md:scale-100 z-10 opacity-80'
                  }
                `}
                >
                  <div
                    className={`
                    w-10 h-10 md:w-14 md:h-14 rounded-full flex items-center justify-center font-serif text-sm md:text-xl font-bold shadow-2xl border-2
                    ${
                      player.team === 'A'
                        ? 'bg-gradient-to-br from-red-900 to-red-700 border-red-400'
                        : 'bg-gradient-to-br from-blue-900 to-blue-700 border-blue-400'
                    }
                ...
                {isMe ? (
                  ...
                  <div className="flex flex-col items-center -space-y-3 md:-space-y-4 scale-90 opacity-60">
                    <div className="flex gap-0.5 md:gap-1">
                      {player.faceDown.map((_, i) => (
                        <Card key={i} faceDown isSmall />
                      ))}
                    </div>
                    <div className="flex gap-0.5 md:gap-1 z-10">
                      {player.faceUp.map((c, i) => (
                        <Card key={i} card={c} isSmall />
                      ))}
                    </div>
                    <div className="flex gap-0.5 md:gap-1 z-20">
                      {player.hand.map((_, i) => (
                        <Card key={i} faceDown isSmall />
                      ))}
                    </div>
          ...
          <div className="w-32 h-32 md:w-64 md:h-64 relative flex items-center justify-center">
            ...
            {gameState.trick.map((play, i) => {
              ...
              // Smaller transform offset on mobile (40px) vs desktop (60px)
              return (
                <div
                  key={i}
                  className="absolute animate-deal"
                  style={{
                    transform: `rotate(${rotations[relIdx]}deg) translateY(-40px) scale(0.8)`,
                  }}
                >
                  <div className="md:scale-125 origin-center">
                    <Card card={play.card} />
                  </div>
                </div>
              );
          ...
          <div className="absolute inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 animate-in fade-in p-4">
            <div className="glass-panel p-4 md:p-8 rounded-2xl text-center border-gold border-2 max-w-sm w-full">
              {gameState.bid.winnerIndex === mySeatIndex ? (
                <div>
                  <h2 className="text-xl md:text-3xl font-serif text-gold mb-4 md:mb-6">
                    Choose Master Suit
                  </h2>
                  <div className="flex gap-2 md:gap-4 justify-center">
                    {SUITS.map((s) => (
                      <button
                        ...
                        className="bg-white p-2 md:p-4 rounded-xl hover:scale-110 transition shadow-lg"
                      >
                        {getSuitIcon(s, 32)}
                      </button>
                    ))}
                ...
                <div>
                  ...
                  <div className="text-gold mb-4 md:mb-6">
                    Current High: {gameState.bid.currentHighBid}
                  </div>
                  <div className="grid grid-cols-4 gap-2 mb-4">
                    {[5, 6, 7, 8].map((n) => (
                      <button
                        key={n}
                        disabled={n <= gameState.bid.currentHighBid}
                        onClick={() => makeBid(n)}
                        className="bg-gold text-black font-bold py-2 md:py-3 rounded hover:bg-yellow-300 disabled:opacity-20 transition"
                      >
                        {n}
                      </button>
                    ))}
                  ...
                  <button
                    onClick={() => makeBid(0)}
                    className="w-full bg-white/10 hover:bg-red-900/50 py-2 md:py-3 rounded text-red-300 border border-red-900/30"
                  >
                    PASS
                  </button>
                </div>
